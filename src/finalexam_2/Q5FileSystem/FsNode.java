 ...
 * <ul>
 * <li>directories, which have child nodes for every file and directory
 *     contained in the directory in the file system that the node
 *     represents</li>
 * <li>text files, which are files whose name ends with ".txt", and
 *     which are therefore assumed to contain text data</li>
 * <li>text files - all files that are not text files</li>
 * </ul></p>
 * <p>
 * FsNodes should support the following default notion of equality:
 * <ul>
 *     <li>Directories are equal if their names are equal and
 *         all contained files and directories are equal</li>
 *     <li>Text files are equal if their names are equal and
 *         they contain the same text - that is, all their
 *         lines of text are equal</li>
 *     <li>Non-text files are equal if their names are equal</li>
 * </ul>
 * By names being equal, we mean the direct name of a file or
 * directory, not its path (that is, names of parents should not
 * be included). The getName() instance method of the file class
